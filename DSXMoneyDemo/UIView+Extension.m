...
- (void)showAlertWithTitle:(NSString *)title message:(NSString *)message  cancelTitle:(NSString *)cancelTitle  confirmTitle:(NSString *)confirmTitle cancelHandler:(void(^)(UIAlertAction *cancelAction))cancelhandler confirmHandler:(void(^)(UIAlertAction *confirmAction))handler {
...
-(void)getLineViewwithPoint:(CGPoint)point{
    ...
    UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(point.x, point.y, SCREEN_WIDTH, DSXSeperatorLineHeight)];
    lineView.backgroundColor = DSXSeparatorColor;
...
- (CAGradientLayer *)setGradualChangingColor:(UIView *)view{
    ...
    gradientLayer.colors = @[(__bridge id)DSXColor(253, 130, 55, 1).CGColor,(__bridge id)DSXColor(249, 80, 67, 1).CGColor];
